...
As shown in the above graph, we found some abnormal records of air temperature. First, there are couple records indicate the air temperature is over 104°F, we consider these as abnormal temperature since there is not such high temperature has ever happened in Boston (the record high temperature in Boston is 104°F, recorded July 4, 1911). We also found lots of temperature records were at 70°F to 104°F. Since most of the Celtics games are in Fall/Winter and early Spring, we thought these records could not represent the actual temperature in Boston area during those game days. We found the average temperature in Boston in October is 62°F/47°F, and average temperature in April is 56°F/41°F. Therefore, we set the threshold at 70°F, hence we remove the records that has temperature higher than that.
...
As we explored the temperature distribution of Red Sox games, we found extremely high temperature of 999°F on June 16, 2016, 675.7°F on July 25, 2016 and so on. Since the record high temperature on the earth is 134.1°F, we seriously consider these records invalid. As we stated before, the record high temperature in Boston is 104°F, we set our threshold here, and we removed the records higher than that.
