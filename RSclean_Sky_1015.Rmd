...
filter the data 'year', 'date', 'X2'(host or guest),  'D/N' (Day/Night?), 'Attendance','Original schedule'

```{r}
#reformat the date as Day_of_Week, Month, DayofMonth
RedSox2012_2017 <- select(RedSox2012_2017,year,Date,X2,D.N,Attendance,Orig..Scheduled) %>% 
...
#reformat orifinal schedued as factor 0 or 1
...
Basket2012_2017 <- Basket2012_2017 %>% select(Date, Day_of_Week,ATTENDANCE)
```
...
```{r}
BuoyRedsox <- read_csv("BuoyRedsox.csv")
BuoyBasket <- read_csv("BuoyBasket.csv")
```
